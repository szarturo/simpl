CREATE OR REPLACE FORCE VIEW V_SIM_TABLA_AMORT_CONCEPTO ("CVE_GPO_EMPRESA", "CVE_EMPRESA", "ID_PRESTAMO", "NUM_PAGO_AMORTIZACION", "FECHA_AMORTIZACION", "ID_ACCESORIO", "IMP_NETO", "IMP_ORIGINAL", "IMP_PAGADO", "CVE_CONCEPTO")
AS
  (SELECT T.CVE_GPO_EMPRESA,
    ...
    CASE
      WHEN C.CVE_CONCEPTO = 'CAPITA'
      THEN ROUND( (NVL(T.IMP_CAPITAL_AMORT,0) + NVL(T.IMP_EXT_CAPITAL_AMORT,0)) * -1, 2)
      WHEN C.CVE_CONCEPTO = 'INTERE'
      THEN ROUND( (NVL(T.IMP_INTERES,0) + NVL(T.IMP_EXT_INTERES,0)) *-1,2)
      WHEN C.CVE_CONCEPTO = 'IVAINT'
      THEN ROUND( (NVL(T.IMP_IVA_INTERES,0) + NVL(T.IMP_EXT_IVA_INTERES,0)) *-1,2)
      WHEN C.CVE_CONCEPTO = 'INTEXT'
      THEN ROUND( (NVL(T.IMP_INTERES_EXTRA,0) + NVL(T.IMP_EXT_INTERES_EXTRA,0)) *-1,2)
      WHEN C.CVE_CONCEPTO = 'IVAINTEX'
      THEN ROUND( (NVL(T.IMP_IVA_INTERES_EXTRA,0) + NVL(T.IMP_EXT_IVA_INTERES_EXTRA,0)) *-1,2)
      WHEN C.CVE_CONCEPTO = 'PAGOTARD'
      THEN ROUND( (NVL(T.IMP_PAGO_TARDIO,0) + NVL(T.IMP_EXT_PAGO_TARDIO,0)) *-1,2)
      WHEN C.CVE_CONCEPTO = 'INTMORA'
      THEN ROUND( (NVL(T.IMP_INTERES_MORA,0) + NVL(T.IMP_EXT_INTERES_MORA,0)) *-1,2)
      WHEN C.CVE_CONCEPTO = 'IVAINTMO'
      THEN ROUND( (NVL(T.IMP_IVA_INTERES_MORA,0) + NVL(T.IMP_EXT_IVA_INTERES_MORA,0)) *-1,2)
    END AS IMP_ORIGINAL,
    CASE
      WHEN C.CVE_CONCEPTO = 'CAPITA'
  ...
  SELECT T.CVE_GPO_EMPRESA,
    ...
    ROUND( (IMP_ACCESORIO - NVL(IMP_EXT_ACCESORIO,0)) * -1,2)                               AS IMP_ORIGINAL,
